...
async function callClaude(system,userContent,maxTokens=1000){
  try{
    const res=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:maxTokens,system,messages:[{role:"user",content:userContent}]})});
...
export default function WardrobeApp(){
  ...
  async function onFileSelected(e){
    ...
    if(cropTarget==="add"){
      ...
      try{
        ...
        setAddForm(f=>({...f,name:parsed.name||f.name,brand:parsed.brand||f.brand,color:parsed.color||f.color,material:parsed.material||f.material,category:parsed.category||f.category,season:parsed.season||f.season,sleeveLength:parsed.sleeveLength||f.sleeveLength,length:parsed.length||f.length,price:parsed.price||f.price,datePurchased:parsed.datePurchased||f.datePurchased}));
  ...
  function addItem(){if(!addForm.name)return;if(addForm.brand)addBrand(addForm.brand);persist([...items,buildItem(addForm)]);setAddForm(emptyForm());setView("closet")}
  ...
  async function scanReceipt(e){
    ...
    try{
      const text=await callClaude(RECEIPT_PROMPT,[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:base64}},{type:"text",text:"Extract all clothing items from this receipt."}],1000);
  ...
  async function generateOutfits(){if(items.length<2)return;setLoadingOutfit(true);setOutfitResult("");try{setOutfitResult(await callClaude(STYLE_SYSTEM,OUTFIT_PROMPT(items,occasion),1000))}catch{setOutfitResult("Error. Try again.")}setLoadingOutfit(false)}
  ...
  return(<div style={{minHeight:"100vh",background:"#111",color:"#e8e2d8",fontFamily:"Georgia, 'Times New Roman', serif",maxWidth:480,margin:"0 auto"}}>
    ...
    {view==="closet"&&(<div>
      ...
      {filtered.length===0?(<div style={{textAlign:"center",padding:"60px 24px",color:"#444",fontFamily:"'DM Sans', system-ui, sans-serif"}}><div style={{fontSize:12,letterSpacing:3,textTransform:"uppercase",marginBottom:8}}>{items.length>0?"No matches":"Nothing here yet"}</div><div style={{fontSize:11,color:"#333"}}>{items.length>0?"Try different filters":"Add your first piece"}</div></div>):(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:2}}>{filtered.map(item=>(<div key={item.id} onClick={()=>evaluateItem(item)} style={{position:"relative",aspectRatio:"3/4",background:"#1a1a1a",cursor:"pointer",overflow:"hidden"}}>{item.imageData?<img src={item.imageData} alt={item.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{width:"100%",height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,padding:12,boxSizing:"border-box"}}>{item.color&&<div style={{fontSize:10,color:"#555"}}>{item.color}</div>}<div style={{fontSize:11,color:"#444",textAlign:"center"}}>{item.name}</div></div>}<div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent, rgba(8,8,8,0.94))",padding:"20px 10px 9px",fontFamily:"'DM Sans', system-ui, sans-serif"}}>{item.name&&<div style={{fontSize:11,fontWeight:500,marginBottom:1,lineHeight:1.3}}>{item.name}</div>}<div style={{fontSize:9,color:"#666",letterSpacing:1,textTransform:"uppercase"}}>{item.brand||item.category}</div></div>{!item.wornDates?.length&&<div style={{position:"absolute",top:7,right:7,background:"#b8976a",color:"#111",borderRadius:2,padding:"2px 5px",fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:8,letterSpacing:1.5,fontWeight:700,textTransform:"uppercase"}}>New</div>}{item.wornDates?.length>0&&<div style={{position:"absolute",top:7,left:7,background:"#ffffff12",color:"#ffffff55",borderRadius:2,padding:"2px 6px",fontFamily:"'DM Sans', system-ui, sans-serif",fontSize:9}}>×{item.wornDates.length}</div>}</div>))}</div>)}
    </div>)}
    ...
    {view==="add"&&(<div style={{padding:24,fontFamily:"'DM Sans', system-ui, sans-serif"}}>
      <div style={{display:"flex",gap:8,marginBottom:24}}>{navBtn("📷 Photo",addMode==="photo",()=>setAddMode("photo"))}{navBtn("🧾 Receipt",addMode==="receipt",()=>{setAddMode("receipt");setReceiptData(null)})}</div>
      {addMode==="photo"&&(<>{scanningImage&&<div style={{textAlign:"center",padding:"16px 0",color:"#b8976a",fontFamily:"'DM Sans', system-ui, sans-serif"}}><div style={{fontSize:11,letterSpacing:2,textTransform:"uppercase"}}>✦ Reading image...</div><div style={{fontSize:10,color:"#555",marginTop:4}}>Extracting brand, color, details</div></div>}<FormFields form={addForm} setForm={setAddForm} onImageClick={()=>openFilePicker("add")} brands={brands} onAddBrand={addBrand}/><button onClick={addItem} disabled={!addForm.name||scanningImage} style={{width:"100%",background:addForm.name&&!scanningImage?"#e8e2d8":"#1a1a1a",color:addForm.name&&!scanningImage?"#111":"#444",border:"none",borderRadius:3,padding:"14px",fontSize:11,letterSpacing:3,textTransform:"uppercase",cursor:addForm.name&&!scanningImage?"pointer":"not-allowed",fontWeight:600,marginTop:8}}>Add to Closet</button></>)}
