...
lines, structure, good fabric, neutral palette. Wide-leg or straight-leg pants, defined waist, structured dresses. Supportive flat shoes only. Real-life elevated — errands, boat, travel. Be direct, opinionated, no fluff.`;
...
export default function WardrobeApp(){
  ...
  async function onFileSelected(e){
    ...
    if(cropTarget==="add"){
      setScanningImage(true);
      try{const base64=dataUrl.split(",")[1];const text=await callClaude(IMAGE_SCAN_PROMPT,[{type:"image",source:{type:"base64",media_type:file.type||"image/jpeg",data:base64}},{type:"text",text:"Analyze this clothing item and extract all details."}],500);const parsed=JSON.parse(text.replace(/```json|```/g,"").trim());setAddForm(f=>({...f,name:parsed.name||f.name,brand:parsed.brand||f.brand,color:parsed.color||f.color,material:parsed.material||f.material,category:parsed.category||f.category,season:parsed.season||f.season,sleeveLength:parsed.sleeveLength||f.sleeveLength,length:parsed.length||f.length,price:parsed.price||f.price,datePurchased:parsed.datePurchased||f.datePurchased}));if(parsed.brand)addBrand(parsed.brand)}catch{}
      setScanningImage(false);
    }
    setCropSrc(dataUrl);
  }
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:2}}>
              {filtered.map(item=>(
                <div key={item.id} onClick={()=>evaluateItem(item)} style={{position:"relative",aspectRatio:"3/4",background:"#1a1a1a",cursor:"pointer",overflow:"hidden"}}>
                  {item.imageData?<img src={item.imageData} alt={item.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{width:"100%",height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,padding:12,boxSizing:"border-box"}}>{item.color&&<div style={{fontSize:10,color:"#555"}}>{item.color}</div>}<div style={{fontSize:11,color:"#444",textAlign:"center"}}>{item.name}</div></div>}
                  <div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent, rgba(8,8,8,0.94))",padding:"20px 10px 9px",fontFamily:"'DM Sans', system-ui, sans-serif"}}>{item.name&&<div style={{fontSize:11,fontWeight:500,marginBottom:1,lineHeight:1.3}}>{item.name}</div>}<div style={{fontSize:9,color:"#666",letterSpacing:1,textTransform:"uppercase"}}>{item.brand||item.category}</div></div>
      ...
      {view==="add"&&(
        <div style={{padding:24,fontFamily:"'DM Sans', system-ui, sans-serif"}}>
          ...
          {receiptData&&receiptData.length>0&&(<>
            ...
            {receiptData.map(item=>(<div key={item.tempId} style={{background:"#1a1a1a",border:"1px solid #222",borderRadius:3,padding:14,marginBottom:12}}>
              <div style={{display:"flex",gap:10,marginBottom:10,alignItems:"flex-start"}}>
                <div onClick={()=>openFilePicker(item.tempId)} style={{width:52,height:68,background:"#111",border:"1px dashed #333",borderRadius:3,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,overflow:"hidden"}}>{receiptImages[item.tempId]?<img src={receiptImages[item.tempId]} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{fontSize:20,color:"#333"}}>+</div>}</div>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",gap:6}}>
                    <select value={item.category} onChange={e=>updateRI(item.tempId,"category",e.target.value)} style={{...inputStyle,padding:"8px 10px",fontSize:11,marginBottom:0,flex:1}}>{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select>
      ...
      {selectedItem&&!editing&&(
        <div style={{position:"fixed",inset:0,background:"#0d0d0d",zIndex:100,overflowY:"auto"}}>
          <div style={{padding:24,maxWidth:480,margin:"0 auto",fontFamily:"'DM Sans', system-ui, sans-serif"}}>
            ...
            {selectedItem.wornDates?.length>0&&(<div style={{marginBottom:18}}><div style={{fontSize:9,letterSpacing:2,textTransform:"uppercase",color:"#555",marginBottom:8}}>Wear History</div><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{[...selectedItem.wornDates].reverse().slice(0,12).map((d,i)=><span key={i} style={{fontSize:10,color:"#666",background:"#1a1a1a",padding:"3px 8px",borderRadius:2}}>{d}</span>)}</div></div>)}
