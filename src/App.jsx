...
export default function WardrobeApp(){
  ...
  const [chatFocusItem,setChatFocusItem]=useState(null);
  ...
  async function sendChat(){const msg=chatInput.trim();if(!msg||chatLoading)return;const newHistory=[...chatHistory,{role:"user",content:msg}];setChatHistory(newHistory);setChatInput("");setChatLoading(true);const focusCtx=chatFocusItem?`\n\nThis conversation is focused on a specific item:\n[${chatFocusItem.category}] ${chatFocusItem.name}${chatFocusItem.brand?` / ${chatFocusItem.brand}`:""}${chatFocusItem.color?` / ${chatFocusItem.color}`:""}${chatFocusItem.material?` / ${chatFocusItem.material}`:""}${chatFocusItem.tags?.length?` / Tags: ${chatFocusItem.tags.join(", ")}`:""} (worn ${chatFocusItem.wornDates?.length||0}x)${chatFocusItem.comments?`\nNotes: ${chatFocusItem.comments}`:""}`:""  ;try{const res=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1000,system:CHAT_SYSTEM(items)+focusCtx,messages:newHistory})});const data=await res.json();const reply=data.content?.[0]?.text||"Sorry, something went wrong.";setChatHistory(h=>[...h,{role:"assistant",content:reply}])}catch{setChatHistory(h=>[...h,{role:"assistant",content:"Error. Try again."}])}setChatLoading(false)}
  ...
  return(<div style={{minHeight:"100vh",background:"#111",color:"#e8e2d8",fontFamily:"Georgia, 'Times New Roman', serif",maxWidth:900,margin:"0 auto"}}>
    ...
    <div style={{padding:"28px 24px 18px",borderBottom:"1px solid #222"}}>
      ...
      <div style={{display:"flex",gap:6,marginTop:18,fontFamily:"'DM Sans', system-ui, sans-serif",flexWrap:"wrap"}}>
        ...
        {navBtn("Chat",view==="chat",()=>{setView("chat");setChatFocusItem(null)})}
    ...
    {view==="chat"&&(<div style={{fontFamily:"'DM Sans', system-ui, sans-serif",paddingBottom:80}}>
      {chatFocusItem&&<div style={{padding:"10px 24px",background:"#1a1a1a",borderBottom:"1px solid #222",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"'DM Sans', system-ui, sans-serif"}}><div style={{fontSize:10,color:"#b8976a",letterSpacing:1}}>Focused: <span style={{color:"#e8e2d8"}}>{chatFocusItem.name}</span></div><button onClick={()=>setChatFocusItem(null)} style={{...ghostBtn,fontSize:10,color:"#555"}}>✕ Clear</button></div>}
      <div style={{padding:"16px 24px",display:"flex",flexDirection:"column",gap:12,minHeight:"60vh"}}>
        {chatHistory.length===0&&(<div style={{textAlign:"center",padding:"40px 24px",color:"#444"}}><div style={{fontSize:12,letterSpacing:3,textTransform:"uppercase",marginBottom:8}}>{chatFocusItem?`Styling ${chatFocusItem.name}`:"Ask anything"}</div><div style={{fontSize:11,color:"#333",lineHeight:1.8}}>{chatFocusItem?"How to style it · What to wear it with · Is this worth keeping?":"What am I missing? · What shoes go with my cream jeans? · Build a capsule for a weekend trip"}</div></div>)}
